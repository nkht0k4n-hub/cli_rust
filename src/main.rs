...
struct Opts{
    ...
    #[clap(short,long)]/*オプション用*/
    verbose:bool,

    #[clap(name="FILE")]/*単なる引数用*/
...
fn eval_inner(&self,tokens:&mut Vec<&str>)->Result<i32>{
    ...
    while let Some(token)=tokens.pop(){
        ...
        if let Ok(x)=token.parse::<i32>(){
            ...
            let y=stack.pop().with_context(||format!("invalid syntax at {}",pos))?;
            let x=stack.pop().with_context(||format!("invalid syntax at {}",pos))?;
...
fn main()->Result<()>{
    ...
    if let Some(path)=opts.formula_file{
        let f=File::open(path)?;/*ファイルを開き,成功したらファイルオブジェクトをfに入れて失敗したらプログラムを停止*/
